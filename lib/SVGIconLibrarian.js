...
import { generateIconLibrary, generateMinimizedIconLibrary } from './generateLibrary.js';
...
class SVGIconLibrarian {
  ...
  async buildLibrary(params) {
    ...
    const selected  = selectIconsFromSets(selectIcons, sets, this.config);
    return await this.writeIconLibrary(selected, outputFile, this.config);
  }
  ...
  async writeIconLibrary(selected, outpath) {
    ...
    await outfile.write(
      this.config.minimize
        ? generateMinimizedIconLibrary(selected)
